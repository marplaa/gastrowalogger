...
function noteSaved(jsonData) {
	if (jsonData["status"] == "ok") {
		requestNewData();
		$("#status_bar").text(jsonData["msg"]);
		$("#status_bar").effect( "highlight", {color:"#1A1"}, 2000 );
		setTimeout(function() {$("#status_bar").text("");}, 3000);
	} else {
		$("#status_bar").text(jsonData["msg"]);
		$("#status_bar").effect( "highlight", {color:"#f60"}, 2000 );
		setTimeout(function() {$("#status_bar").text("");}, 3000);
	}
}
