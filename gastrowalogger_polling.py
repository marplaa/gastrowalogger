    ...
    for sensor in sensors:
        try:
            ...
            if last_values[sensor][1] == 0 and value != 0:
                conn.execute("INSERT INTO consumptions (sensor, 'timestamp', 'count') VALUES (?,?,?)",
                             (sensors[sensor]["id"], last_values[sensor][0], 0))

            if value != 0:
                conn.execute("INSERT INTO consumptions (sensor, 'timestamp', 'count') VALUES (?,?,?)",
                             (sensors[sensor]["id"], timestamp, value))
...
def start_polling(database, sb_host, sb_port, interval):
    ...
    print(
        'Started new polling thread (database=' + _database + ", SerialBus-Host=" + _sb_host + ", SerialBus-Port=" + str(
            _sb_port) + ", interval=" + str(interval) + ")")
...
if __name__ == '__main__':
    ...
    start_polling(database, config.get("SERIALBUS", "SERIALBUS_HOST"), config.getint("SERIALBUS", "SERIALBUS_PORT"),
                  config.getint("GASTROWALOGGER", "RECORDING_INTERVAL"))
