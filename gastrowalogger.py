...
import sys
import math
from flask_babel import Babel, gettext, ngettext, Locale
...
#     to_date_time_utc = to_date_time_tz.astimezone(pytz.timezone('UTC'))
    ...
    cur = db.execute("SELECT timestamp, note from notes where sensor = ? and timestamp between ? and ? ORDER BY timestamp", (sensor["id"], from_date_time_utc.timestamp(), to_date_time_utc.timestamp()))
...
@app.route('/sensor/_add_note', methods=['POST'])
def add_note():
    try:
        sensor_name = request.form["sensor"]
        locale = config.get("GASTROWALOGGER", "LOCALE")
        
        date = datetime.combine(parse_date(request.form["note_date"], locale=locale), datetime.min.time())
        local_datetime = tz.normalize(tz.localize(date))
        timestamp = local_datetime.astimezone(pytz.timezone('UTC')).timestamp()
        
        db = get_db()
        cur = db.execute("INSERT INTO notes (timestamp, sensor, note)  VALUES (?, ?, ?)", (timestamp, sensors[sensor_name]["id"], request.form["note"])) 
        db.commit()
        
        return jsonify({"status" : "ok", "msg": gettext("Note saved")})
    except Exception as e:
        if hasattr(e, 'message'):
            logging.error(e.message)
            return jsonify({"status" : "error", "error_msg" : e.message})
        else:
            logging.error(str(e))
